...
Alexis, front-end developer from Mexico. Focus: React JS
layers. Loves to build new things.

Personal website: https://codingcodax.dev
...
When creating branches, prefix them with `codingcodax/` to indicate they came from me.
...
- Always lookup the latest version of an NPM package before adding it as a
  dependency (`bun pm view <package-name> version`).
...
ecosystem via `bunx --bun skills find [query]`.
